...
// ログイン中ユーザーのタスク一覧を取得して、JSONで返す
func (tc *taskController) GetAllTasks(c echo.Context) error {
	// Contextの中にあるjwtを取り出す
	user := c.Get("user").(*jwt.Token)
	// JWTの中身（claims）から user_id を取り出す
	claims := user.Claims.(jwt.MapClaims)
	userId := claims["user_id"]
  // userIdをuintに直してusecaseに返す
	// JWTの claims を MapClaims で扱っているので、数値の多くはfloat64として入ってくるからuintに直した
...
func (tc *taskController) GetTaskById(c echo.Context) error {
	...
	claims := user.Claims.(jwt.MapClaims)
  userId := claims["user_id"]
	// 変数idがstring型なので、Atoiでint型に変換している
	id := c.Param("taskId")
